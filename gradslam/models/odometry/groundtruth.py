import torch
from gradslam.core.geometry import relative_transformation
from gradslam.core.structures import RGBDImages
from .base import Odometry

__all__ = ["GroundTruthOdometry"]


class GroundTruthOdometry(Odometry):
    ...
    def provide(self, rgbdimages1: RGBDImages, rgbdimages2: RGBDImages) -> torch.Tensor:
        ...
        if rgbdimages1.poses is None:
            raise ValueError(
                "Input 1 (rgbdimages1) missing poses. Poses must be provided if using GroundTruthOdometry"
            )
        if rgbdimages2.poses is None:
            raise ValueError(
                "Input 2 (rgbdimages2) missing poses. Poses must be provided if using GroundTruthOdometry"
            )
    ...
    def localize(self, live_frame: RGBDImages, *args, **kwargs):
